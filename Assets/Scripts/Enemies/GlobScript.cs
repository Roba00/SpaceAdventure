...
public class GlobScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (!isDying)
        {
            ...
            if (plyrDist < 10f && !isAttackMode) {isAttackMode = true; StopCoroutine(StillAnimate()); StartCoroutine(Attack());}
            if (plyrDist > 10f && isAttackMode /*&& !isAttacking*/) {isAttackMode = false; StopCoroutine(Attack()); StartCoroutine(StillAnimate());}
        }
    }
